...
import { productService, tradeOfferService, authService } from '../services/api';
...
const Profile = () => {
  ...
  useEffect(() => {
    if (user) {
      ...
      // Address'i parse ederek city ve zipCode alanlarını belirle
      let addressValue = user.address || '';
      let cityValue = '';
      let zipCodeValue = '';
      
      if (addressValue) {
        console.log(`PROFILE CRITICAL - Adres inceleniyor: "${addressValue}"`);
        
        // Address format example: "Main St. 123, New York 10001"
        // Try to find the last comma and extract city and zip code
        const commaIndex = addressValue.lastIndexOf(',');
        
        if (commaIndex !== -1) {
          // There's a comma in the address, extract the parts after comma
          const cityZipPart = addressValue.substring(commaIndex + 1).trim();
          addressValue = addressValue.substring(0, commaIndex).trim();
          
          // Try to extract city and zip code
          const zipRegex = /\b\d{5,6}\b/; // Basic zip code pattern (5-6 digits)
          const zipMatch = cityZipPart.match(zipRegex);
          
          if (zipMatch) {
            // Found zip code
            zipCodeValue = zipMatch[0];
            // Everything before the zip code is the city
            const zipIndex = cityZipPart.indexOf(zipCodeValue);
            if (zipIndex > 0) {
              cityValue = cityZipPart.substring(0, zipIndex).trim();
            } else {
              cityValue = cityZipPart.replace(zipCodeValue, '').trim();
            }
          } else {
            // No zip code found, assume everything is city
            cityValue = cityZipPart;
          }
        }
      }
      
      console.log(`PROFILE CRITICAL - Parsed address values: address="${addressValue}", city="${cityValue}", zipCode="${zipCodeValue}"`);
  ...
  const handlePasswordSubmit = async (e) => {
    ...
    try {
      ...
      setApiError(err.response?.data?.error || err.response?.data?.message || 'Şifre güncellenemedi. Lütfen tekrar deneyin.');
    }
  };
