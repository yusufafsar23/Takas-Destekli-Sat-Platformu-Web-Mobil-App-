...
const ProfileScreen = ({ route, navigation }) => {
  ...
  useEffect(() => {
    ...
    const unsubscribe = navigation.addListener('focus', () => {
      if (isCurrentUser) {
        // Kullanıcı kendi profilindeyse, profil bilgilerini her ekrana dönüşte yenile
        refreshUserData();
      }
    ...
    // Cleanup
    return unsubscribe;
  }, [navigation, user, isCurrentUser]);
  
  // Kullanıcı verilerini yenileme fonksiyonu
  const refreshUserData = async () => {
    try {
      console.log('Profil bilgileri yenileniyor...');
      const storedUser = await AsyncStorage.getItem('user');
      if (storedUser) {
        const parsedUser = JSON.parse(storedUser);
        ...
        console.log('Kullanıcı bilgileri AsyncStorage\'dan güncellendi');
      }
    } catch (error) {
      console.error('Kullanıcı verilerini yenileme hatası:', error);
    }
  };
  ...
  // İsim Soyisim bölümü için düzenleme fonksiyonu
