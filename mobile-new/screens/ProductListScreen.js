...
const ProductListScreen = ({ route, navigation }) => {
    ...
    try {
      ...
      if (response && response.success && Array.isArray(response.data)) {
        ...
        const filteredCategories = response.data.filter(cat => {
          ...
          return !name.includes('aydınlatma') && 
                 !name.includes('bahçe') && 
                 !name.includes('bebek ve çocuk') && 
                 !name.includes('fotoğraf') && 
                 !name.includes('kamera') && 
                 !name.includes('otomobil');
        });
        
        const categoriesData = filteredCategories.map(cat => ({
          ...cat,
        ...
        // Filter out unwanted categories
        const filteredCategories = response.filter(cat => {
          const name = (cat.name || '').toLowerCase();
          return !name.includes('aydınlatma') && 
                 !name.includes('bahçe') && 
                 !name.includes('bebek ve çocuk') && 
        ...
        // Kategori ismine göre kategori seçimi yap
        if (route.params?.categoryName && filteredCategories.length > 0) {
          const categoryNameLower = route.params.categoryName.toLowerCase();
          const foundCategory = filteredCategories.find(cat => 
            cat.name?.toLowerCase() === categoryNameLower
          );
  ...
  const fetchProducts = async () => {
    ...
    try {
      ...
      if (isElektronikFilter || (selectedCategory && CATEGORIES_WITH_SUBCATEGORIES[selectedCategory])) {
        ...
        switch (sortOption) {
          case 'Fiyat Artan':
            const sortedAscProducts = [...productsData].sort((a, b) => {
              const priceA = a.price ? parseFloat(a.price) : 0;
              const priceB = b.price ? parseFloat(b.price) : 0;
              return priceA - priceB;
            });
            productsData = sortedAscProducts;
            console.log('Ürünler artan fiyata göre sıralandı');
            break;
          case 'Fiyat Azalan':
            const sortedDescProducts = [...productsData].sort((a, b) => {
              const priceA = a.price ? parseFloat(a.price) : 0;
              const priceB = b.price ? parseFloat(b.price) : 0;
              return priceB - priceA;
            });
            productsData = sortedDescProducts;
            console.log('Ürünler azalan fiyata göre sıralandı');
            break;
          // En Yeni sıralaması için
          case 'En Yeni':
            const sortedNewProducts = [...productsData].sort((a, b) => {
              const dateA = a.createdAt ? new Date(a.createdAt) : new Date(0);
              const dateB = b.createdAt ? new Date(b.createdAt) : new Date(0);
              return dateB - dateA;
            });
            productsData = sortedNewProducts;
            console.log('Ürünler en yeni tarihe göre sıralandı');
            break;
        }
      }
  ...
  const renderProductItem = ({ item }) => {
    // Fiyat formatını ayarla
    const formatPrice = () => {
      // Eğer ürün sadece takas için ise
      if (item.tradeOnly) {
        return 'Sadece Takas';
      }
      // Veya fiyat 0 ise ve takas kabul ediliyor ise
      if ((!item.price || item.price === 0) && item.acceptsTradeOffers) {
        return 'Sadece Takas';
      }
