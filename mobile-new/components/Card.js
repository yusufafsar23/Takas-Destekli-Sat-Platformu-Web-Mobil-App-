 ...
 * @param {boolean} tradeOnly - Ürün sadece takas için mi?
 * @param {boolean} acceptsTradeOffers - Takas kabul ediliyor mu?
 */
export const Card = ({
  children,
  title,
  ...
  const formatPrice = () => {
    ...
    if ((!price || price === 0) && acceptsTradeOffers) {
      return 'Sadece Takas';
    }
  ...
  if (imageUrl && (title || price !== undefined)) {
    return (
      <CardComponent
